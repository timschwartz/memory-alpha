...
- **FR-002a**: The wikitext renderer MUST sanitize its HTML output before DOM insertion, allowlisting only safe tags (h1–h6, p, a, ul, ol, li, table, thead, tbody, caption, tr, td, th, strong, em, b, i, mark, span, div, dl, dt, dd, blockquote, pre, code, br, hr, sub, sup) and safe attributes (href, target, rel, class, id, colspan, rowspan, scope, alt, title), stripping all other tags and attributes as a defense-in-depth measure.
- **FR-002b**: External links rendered from wikitext MUST open in a new browser tab (`target="_blank"`) with `rel="noopener"` to prevent reverse tabnapping.
